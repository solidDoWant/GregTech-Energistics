...
public class StockerTerminalContainer extends AEBaseContainer {
	...
	private final Map<CoverAE2Stocker, InvTracker> diList = new HashMap<>();
	...
	public void doAction(final EntityPlayerMP player, final InventoryAction action, final int slot, final long id) {
		...
		if (inv != null) {
			...
			final IItemHandler theSlot = new WrapperFilteredItemHandler(
					new WrapperRangeItemHandler(inv.server.itemHandler, slot, slot + 1), new PatternSlotFilter());
	...
	private static class InvTracker {
		...
		private final SideDetails client;
		private final SideDetails server;
		private final CoverAE2Stocker cover;
		...
		public static InvTracker build(final CoverAE2Stocker cover) {
			...
			SideDetails client = new SideDetails(new AppEngInternalInventory(null, patterns.getSlots()), 0, 0, null);
	...
	private static class SideDetails {
		public IItemHandler itemHandler;
		public long stockAmount;
		public long availableAmount;
		public CoverAE2Stocker.CoverStatus status;

		public SideDetails(IItemHandler itemHandler, long stockAmount, long availableAmount, CoverAE2Stocker.CoverStatus status) {
		...
		public void update(long stockAmount, long availableAmount, CoverAE2Stocker.CoverStatus status) {
			this.stockAmount = stockAmount;
			this.availableAmount = availableAmount;
			this.status = status;
		}
	...
	private String getItemTagName(InvTracker tracker) {
		return '=' + Long.toString(tracker.which, Character.MAX_RADIX);
	}

	private NBTTagCompound getItemTag(NBTTagCompound parentTag, InvTracker tracker) {
	...
	private void updateTagChildItems(NBTTagCompound data, InvTracker tracker, int offset, int length) {
	...
	private void setItemTag(NBTTagCompound parent, NBTTagCompound child, InvTracker inv) {
		if (child.hasNoTags())
			return;
	...
	private void updateTagCoverStatus(NBTTagCompound data, InvTracker inv) {
	...
	private void buildStockerTag(final NBTTagCompound data, final Entry<CoverAE2Stocker, InvTracker> en,
	...
	public static StockerTerminalContainer getServerGuiContainer(AEPartLocation side, EntityPlayer player, World world,
		...
		StockerTerminalContainer container = new StockerTerminalContainer(player.inventory, (StockerTerminalPart) part);
